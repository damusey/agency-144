...
export default function ProcessSection() {
  ...
  return (
    <section style={{ background: 'var(--surface-alt)', borderTop: '1px solid var(--border)', borderBottom: '1px solid var(--border)', position: 'relative' }}>
      ...
      <div className="wrap grid-stack-mob" style={{ display: 'grid', gridTemplateColumns: 'minmax(20vw, 1fr) 1.6fr', gap: '8vw', paddingBottom: '10vw', alignItems: 'start' }} ref={containerRef}>
        ...
        <div className="hidden lg:block hidden-mobile" style={{ position: 'sticky', top: '25vh' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '48px', position: 'relative' }}>
            ...
            {/* The Track */}
            <div style={{ position: 'absolute', left: '23px', top: '0', bottom: '0', width: 'min(2px, 100vw)', background: 'rgba(255,255,255,0.05)', zIndex: 0 }} />
            
            {/* The Neon Fluid fill */}
            <motion.div style={{ position: 'absolute', left: '23px', top: '0', bottom: '0', width: 'min(2px, 100vw)', background: 'var(--brand)', scaleY: scrollYProgress, transformOrigin: 'top', filter: 'drop-shadow(0 0 10px rgba(255,92,0,0.8))', zIndex: 1 }} />
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'clamp(10vh, 15vh, 30vh)', padding: 'clamp(4vh, 8vh, 10vh) 0 clamp(10vh, 20vh, 30vh)' }}>
          {steps.map((s, i) => {
            const isActive = activeStep === i;
            return (
              <div key={i} style={{ opacity: isActive ? 1 : 0.3, transition: 'opacity 0.5s ease', transform: isActive ? 'scale(1)' : 'scale(0.98)' }}>
                ...
                <h3 style={{ fontSize: 'clamp(24px, 3.5vw, 48px)', fontWeight: 800, color: 'var(--ink)', marginBottom: 'clamp(20px, 3vw, 32px)', letterSpacing: '-1.5px', lineHeight: 1.1 }}>{s.t}</h3>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                  {s.d.map((paragraph, idx) => (
                    <p key={idx} style={{ fontSize: 'clamp(14px, 1.4vw, 20px)', color: 'var(--ink2)', lineHeight: 1.6, maxWidth: '700px', margin: 0, fontWeight: idx === s.d.length - 1 && i === steps.length - 1 ? 600 : 400 }}>
                      {paragraph}
                    </p>
                  ))}
                </div>
